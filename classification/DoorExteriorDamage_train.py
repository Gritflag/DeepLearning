...
def train_validate(model, trainloader, valloader, criterion, optimizer, writer, device, title="train", epochs=10, ):
    """모델을 훈련시키고 검증합니다."""
    for epoch in range(epochs):
        ...
        # 모델 상태를 파일로 저장
        torch.save(model.state_dict(), f"{title}_epoch_{epoch+1}.pth")
...
if __name__ == "__main__":
    ...
    for i, model_name in enumerate(model_names):
        ...
        train_validate(model, trainloader, valloader, criterion, optimizer, writer, device, titles[i], epochs=100)
        writer.close()
